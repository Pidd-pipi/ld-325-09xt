...
import type { DemoRole, Product, Trend } from '@/lib/types';
...
import { SupplierWorkbench } from '@/components/supplier/SupplierWorkbench';
import { AdminReviewQueue } from '@/components/supplier/AdminReviewQueue';
...
export default function Home() {
  ...
  useEffect(() => {
    if (selected) {
      api.trend(selected.ID, trendRange).then(setTrend).catch(() => setTrend(null));
    }
  }, [selected, trendRange]);
  ...
  const visible = useMemo(() => products
    ...
    .sort((a, b) => sort === 'price'
      ...
      : sort === 'sales'
        ...
        : b.Rating - a.Rating), [products, category, query, sort]);

  const compare = (id: number) => setCompareIDs((current) => current.includes(id)
  ...
  return (
    <main id="top">
      <AppHeader query={query} onQuery={setQuery} role={role} onRole={setRole} />
      <section className="hero">
        <div>
          ...
          <div className="hero-note"><ShieldCheck size={18} /><span>改价审核 · 降价提醒 · 30 天历史趋势 · 真实可比较字段</span></div>
        </div>
        ...
        <>
          ...
          <InsightPanel
            ...
            onAlert={async (id, target) => {
              ...
              notify('价格预警已建立，可在「价格提醒」查看触发价与状态');
            }}
          />
          <AlertCenter products={products} />
          <SupplierWorkbench products={products} />
          {role === 'admin' && <AdminReviewQueue />}
          <BudgetCalculator
