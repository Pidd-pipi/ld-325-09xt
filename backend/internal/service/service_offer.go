...
func (s *OfferService) ListMine(supplierID uint) ([]model.Offer, error) {
	rows, err := s.repo.ListBySupplier(supplierID)
	if err != nil {
		return nil, fmt.Errorf("list supplier offers service: %w", err)
	}
	return rows, nil
}
